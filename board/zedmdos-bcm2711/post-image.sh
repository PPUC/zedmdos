...
cp "${BR2_EXTERNAL_ZEDMDOS_PATH}/board/zedmdos-bcm2711/cmdline.txt"  "${BINARIES_DIR}/zedmdos/" || exit 1
cp "${BR2_EXTERNAL_ZEDMDOS_PATH}/board/zedmdos-bcm2711/config.txt"   "${BINARIES_DIR}/zedmdos/" || exit 1
...
cd "${BINARIES_DIR}/zedmdos" && zip -qr "${BINARIES_DIR}/zedmdos.zip" * || exit 1
echo "${BINARIES_DIR}/zedmdos.zip" >&2
